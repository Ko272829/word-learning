...
你是英语学习产品的音标编辑。请为下面这些词条补充常见、美式英语为主的音标。
...
3. 每个词条都必须返回一条音标。
4. phonetic 字段必须带斜杠，例如 /həˈloʊ/。
5. 如果是短语，也尽量返回适合学习者记忆的整条音标。
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!Array.isArray(words) || words.length === 0) {
      return json({ error: "请提供需要补音标的词条" }, 400);
    }
    ...
    const upstream = await fetch("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "你是英语学习音标生成助手。必须返回合法 JSON，不要输出 Markdown 代码块之外的解释。"
          },
          {
            role: "user",
    ...
    if (!content) {
      return json({ error: "DeepSeek 未返回音标内容" }, 502);
    }

    const parsed = extractJsonObject(content);
    if (!Array.isArray(parsed.phonetics) || parsed.phonetics.length === 0) {
      return json({ error: "返回的音标格式无效" }, 502);
    }

    const phonetics = parsed.phonetics
      .map(sanitizePhonetic)
      .filter((item) => item.word && item.meaning && item.phonetic);

    if (phonetics.length === 0) {
      return json({ error: "AI 未生成有效音标" }, 502);
    }

    return json({ phonetics });
  } catch (error) {
    if (error.name === "AbortError") {
      return json({ error: "补音标请求超时，请稍后重试。" }, 504);
    }
